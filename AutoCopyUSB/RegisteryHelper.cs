...
namespace AutoCopyUSB
{
    internal class RegisteryHelper
    {
        ...
        public static void SetStartupEnabled(bool enabled)
        {
            ...
            if (enabled)
            {
                rk.SetValue(Utils.ApplicationName(), Application.ExecutablePath.ToString());
            }
            else
            {
        ...
        {
            RegistryKey rk = Registry.CurrentUser.OpenSubKey(STARTUP_KEY, true);
            var value = rk.GetValue(STARTUP_KEY);
            return value != null && value.ToString().Equals(Application.ExecutablePath.ToString());
        }
    }
}
